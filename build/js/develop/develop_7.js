  ...
  function init(){       
    ...
    var myPlacemark = new ymaps.Placemark([55.8721, 48.8821] , {},
        { iconLayout: 'default#image',
          iconImageHref: '/wp-content/themes/rainbow/images/marker-map.png',
...
/*  var content_width = $('.acc-title').next('.acc-content').css('width');

  $('.acc-content:not(:first)').css('width', '0');
  $('.acc-title:first').addClass('selected');

  $('.acc-title').hover(function(){

    if($(this).next().css('width') == '0px'){
      $('.acc-content').stop().animate({
        width:'0px'
      });
      $(this).next('.acc-content').stop().animate({ 
        width: content_width
      });

      $('.acc-title').removeClass('selected');
